...
import {
	type ChatLocation,
	localStorage,
} from "../../services/LocalStorageProvider";
import {
...
export class ChatsController implements vscode.Disposable {
	...
	public registerViewsAndCommands() {
		...
		this.disposables.push(
			...
			vscode.commands.registerCommand("cody.chat.history.export", () =>
				this.exportHistory(),
			),
			vscode.commands.registerCommand("cody.chat.history.clear", (arg) =>
				this.clearHistory(arg),
			),
			vscode.commands.registerCommand("cody.chat.history.delete", (item) =>
			...
			vscode.commands.registerCommand(
				...
				(args: { text: string }) =>
					handleCodeFromSaveToNewFile(args.text, this.options.editor),
			),
		);
	}
	...
	private async sendSmartApplyResultToChat(
	...
	private async passthroughVsCodeOpen(...args: unknown[]): Promise<void> {
		if (args[1] && (args[1] as any).viewColumn === vscode.ViewColumn.Beside) {
			...
			(args[1] as any).viewColumn = textEditor
	...
	private async exportHistory(): Promise<void> {
		...
		if (authStatus.authenticated) {
			try {
				...
				const logContent = new TextEncoder().encode(
	...
	private async createEditorChatController(
		...
		if (panel) {
			...
			const activePanelViewColumn = this.activeEditor?.webviewPanelOrView
				...
				: undefined;
			await chatController.createWebviewViewOrPanel(
		...
		if (chatController.webviewPanelOrView) {
			webviewViewOrPanelOnDidChangeViewState(chatController.webviewPanelOrView)(
				(e) => {
...
function getNewChatLocation(): ChatLocation {
	const chatDefaultLocation =
		vscode.workspace
			.getConfiguration()
			.get<"sticky" | "sidebar" | "editor">("cody.chat.defaultLocation") ??
		"sticky";
